...
namespace Alura.Loja.Testes.ConsoleApp
{
    ...
    {
        private SqlConnection conexao; //connection to the database

        public ProdutoDAO()
        {
            this.conexao = new SqlConnection("Server=(localdb)\\mssqllocaldb;Database=LojaDB;Trusted_Connection=true;"); // path to the databse in MSSQL
            this.conexao.Open(); //method to open database connection
        }
        ...
        internal void Adicionar(Produto p) 
        {
            try
            {
                ...
                insertCmd.CommandText = "INSERT INTO Produtos (Nome, Categoria, Preco) VALUES (@nome, @categoria, @preco)"; //adding products to the database using query
        ...
        internal void Atualizar(Produto p)
        {
            try
            {
                ...
                updateCmd.CommandText = "UPDATE Produtos SET Nome = @nome, Categoria = @categoria, Preco = @preco WHERE Id = @id"; //updating products in the database using query
        ...
        internal void Remover(Produto p)
        {
            try
            {
                ...
                deleteCmd.CommandText = "DELETE FROM Produtos WHERE Id = @id"; //deleting products to the database using query
